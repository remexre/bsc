...
initCState :: String -> Flags -> Bool ->
              [ADef] -> [AVInst] -> [RuleTriple] -> IO CState
initCState str flags doHardFail ds avis rs = do
  ctx <- C.mkContext
  when traceTest $ do
    ver <- C.version ctx
    putStrLn $ "Using cvc5 version: " ++ ver
  falseE <- C.mkFalse ctx
  return (CState { context = ctx,
                   ...
                   defCExprMap = M.empty,
                   portCExprMap = M.empty,
                   expCExprMap = M.empty
                 })


addADefToCState :: CState -> [ADef] -> IO CState
addADefToCState s ds = do
  let defmap  = defMap s
      defmap' = map_insertMany [(id, d) | d@(ADef id _ _ _) <- ds] defmap
  return (s { defMap = defmap' })

withTraceTest :: PPrint a => CM a -> CM a
withTraceTest m =
  if traceTest
  then withElapsed $ do
         res <- m
...
checkDisjointExpr :: CState -> AExpr -> AExpr -> IO (Maybe Bool, CState)
checkDisjointExpr s e1 e2 = runStateT (checkDisjointExprM e1 e2) s

checkDisjointExprM :: AExpr -> AExpr -> CM (Maybe Bool)
checkDisjointExprM e1 e2 = withTraceTest $ do
  ...
  let doProof = do
        ...
        --
        (ye1, _) <- convAExpr2CExpr_Force True e1
        --when traceTest $ traceM("conv1 done")
        (ye2, _) <- convAExpr2CExpr_Force True e2
        --when traceTest $ traceM("conv2 done")

        liftIO $ C.assert ctx ye1
        liftIO $ C.assert ctx ye2
  ...
  return $ case (sat_res) of
              C.Invalid -> Just False
              C.Valid   -> Just True
              C.Timeout -> Nothing
              C.Error   -> internalError ("CVC5 query error")

checkDisjointRulePair :: CState -> (ARuleId, ARuleId) -> IO (Maybe Bool, CState)
checkDisjointRulePair s rp = runStateT (checkDisjointRulePairM rp) s

checkDisjointRulePairM :: (ARuleId, ARuleId) -> CM (Maybe Bool)
...
checkBiImplication :: CState -> AExpr -> AExpr -> IO ((Bool, Bool), CState)
checkBiImplication s e1 e2 = runStateT (checkBiImplicationM e1 e2) s

checkBiImplicationM :: AExpr -> AExpr -> CM (Bool, Bool)
...
checkImplication :: C.Expr -> C.Expr -> CM Bool
{-
-- cvc5 has a built-in implication operator, so use that instead of this
checkImplication ye1 ye2 = do
  ctx <- gets context
  -- If (!a || b) is True, then a implies b
  not_ye1 <- liftIO $ C.mkNot ctx ye1
  yor <- liftIO $ C.mkOr ctx not_ye1 ye2
  when traceTest $ traceM("checking if implication is true")
  isTrueCExpr yor >>= return . fromMaybe False
-}
checkImplication ye1 ye2 = do
  ctx <- gets context
  yimp <- liftIO $ C.mkImplies ctx ye1 ye2
  when traceTest $ traceM("checking if implication is true")
  isTrueCExpr yimp >>= return . fromMaybe False

isConstExpr :: CState -> AExpr -> IO (Maybe Bool, CState)
isConstExpr s e = runStateT (isConstExprM e) s

isConstExprM :: AExpr -> CM (Maybe Bool)
isConstExprM e = withTraceTest $ do

  when traceTest $ traceM("checking is-constant: " ++ ppString e)
  when traceConv $ traceM("   e = " ++ show e)
  (ye, _) <- convAExpr2CExpr_Force True e
  isConstCExpr ye

isConstCExpr :: C.Expr -> CM (Maybe Bool)
isConstCExpr ye = do
  is_true <- isTrueCExpr ye
  case (is_true) of
    Just True -> return $ Just True
    otherwise -> do is_false <- isFalseCExpr ye
                    case (is_false) of
                      Just True -> return $ Just False
                      otherwise -> return Nothing

isTrueCExpr :: C.Expr -> CM (Maybe Bool)
isTrueCExpr ye = do
  ctx <- gets context
  not_ye <- liftIO $ C.mkNot ctx ye
  isFalseCExpr not_ye

isFalseCExpr :: C.Expr -> CM (Maybe Bool)
isFalseCExpr ye = do
  let doProof = do
        ctx <- gets context
        liftIO $ C.ctxPush ctx
        liftIO $ C.assert ctx ye
        --
        yf <- gets falseExpr
        sat_res <- liftIO $ C.query ctx yf
        liftIO $ C.ctxPop ctx
...
checkEq :: CState -> AExpr -> AExpr -> IO (Maybe Bool, CState)
checkEq s e1 e2 = runStateT (checkEqM e1 e2) s

checkNotEq :: CState -> AExpr -> AExpr -> IO (Maybe Bool, CState)
checkNotEq s e1 e2 = runStateT (checkNotEqM e1 e2) s

checkEqM :: AExpr -> AExpr -> CM (Maybe Bool)
checkEqM e1 e2 = withTraceTest $ do
  ...
  (y_e1, ty_e1) <- convAExpr2CExpr Nothing e1
  --when traceTest $ traceM("conv1 done")
  (y_e2, _) <- convAExpr2CExpr (Just ty_e1) e2 >>= convType (Just ty_e1)
  --when traceTest $ traceM("conv2 done")
  ctx <- gets context

  -- STP's mkEq segfaulted on Bool inputs, so use iff (cvc5 handles either)
  let eqFn = if (ty_e1 == SBool) then C.mkIff else C.mkEq
  yeq <- liftIO $ eqFn ctx y_e1 y_e2

  isTrueCExpr yeq


checkNotEqM :: AExpr -> AExpr -> CM (Maybe Bool)
...
data CState =
    CState {
               ...
               proofMap      :: M.Map (AExpr, AExpr) C.Result,
               proofMapS     :: M.Map C.Expr C.Result,
               ...
               defCExprMap   :: M.Map AId (C.Expr, SType),
               portCExprMap  :: M.Map AId (C.Expr, SType),
               -- for expressions which we treat as variables
               expCExprMap   :: M.Map AExpr (C.Expr, SType)
              }
...
-- while cvc5 requires a conversion between the two.
...
addToExpMap :: AExpr -> (C.Expr, SType) -> CM ()
addToExpMap e res = do
    s <- get
    let emap = expCExprMap s
        emap' = M.insert e res emap
    put (s { expCExprMap = emap' })

addToDefMap :: AId -> (C.Expr, SType) -> CM ()
addToDefMap d res = do
    s <- get
    let dmap = defCExprMap s
        dmap' = M.insert d res dmap
    put (s { defCExprMap = dmap' })

addToPortMap :: AId -> (C.Expr, SType) -> CM ()
addToPortMap p res = do
    s <- get
    let pmap = portCExprMap s
        pmap' = M.insert p res pmap
    put (s { portCExprMap = pmap' })

addToProofMap :: AExpr -> AExpr -> C.Result -> CM()
...
{-
assertDef :: AId -> (C.Expr, SType) -> CM (C.Expr, SType)
assertDef i (y_e, ty_e) = do
    ...
    yeq <- liftIO $ C.mkEq ctx y_var y_e
    liftIO $ C.assert ctx yeq
    return var
-}

makeDeclAndVar :: Maybe SType -> String -> Integer -> CM (C.Expr, SType)
...
makeDeclAndVar (Just SBool) str width = do
    ...
    ctx <- gets context
    ty <- liftIO $ C.mkBoolType ctx
    var <- liftIO $ C.mkVar ctx str ty
    return (var, SBool)
makeDeclAndVar _ str width = do
    ctx <- gets context
    ty <- liftIO $ C.mkBitVectorType ctx (fromInteger width)
    var <- liftIO $ C.mkVar ctx str ty
    return (var, SBits width)

addUnknownExpr :: Maybe SType -> AExpr -> Integer -> CM (C.Expr, SType)
addUnknownExpr mty e width = do
    when traceConv $ traceM("addUnknownExpr: " ++ ppString e)
    emap <- gets expCExprMap
...
convType :: Maybe SType -> (C.Expr, SType) -> CM (C.Expr, SType)
...
convType (Just (SBool)) (ye, SBits width) = do
    ...
    yb <- liftIO $ C.mkBVConstantFromInteger ctx 1 1
    yeq <- liftIO $ C.mkEq ctx ye yb
    return (yeq, SBool)
{-
    -- this built-in conversion might be better?
    yconv <- liftIO $ C.mkBVBoolExtract ctx 0 ye
    return (yconv, SBool)
-}
convType (Just (SBits width)) (ye, SBool) = do
    ...
    ctx <- gets context
    -- XXX Can this be memoized?
    yb1 <- liftIO $ C.mkBVConstantFromInteger ctx 1 1
    yb0 <- liftIO $ C.mkBVConstantFromInteger ctx 1 0
    yif <- liftIO $ C.mkIte ctx ye yb1 yb0
    return (yif, SBits width)
{-
    -- cvc5 has a built-in conversion function
    yconv <- liftIO $ C.mkBoolToBitVector ctx ye
    return (yconv, SBits width)
-}

toBool :: (C.Expr, SType) -> CM (C.Expr, SType)
...
toBits :: (C.Expr, SType) -> CM (C.Expr, SType)
...
convAExpr2CExpr :: Maybe SType -> AExpr -> CM (C.Expr, SType)

-- If the context expects Bool, create a Bool value
convAExpr2CExpr (Just SBool) (ASInt _ (ATBit width) (IntLit _ _ ilValue)) = do
    when traceConv $ traceM("conv IntLit (Bool)" ++ itos ilValue)
    when (width /= 1) $
        internalError ("convAExpr2CExpr: invalid width: " ++ ppReadable width)
    ctx <- gets context
    ye <- liftIO $ if (ilValue == 0)
                    then C.mkFalse ctx
                    else if (ilValue == 1)
                         then C.mkTrue ctx
                         else internalError ("convAExpr2CExpr: invalid Bool" ++
                                             ppReadable ilValue)
    return (ye, SBool)
convAExpr2CExpr _ (ASInt _ (ATBit width) (IntLit _ _ ilValue)) = do
    when traceConv $ traceM("conv IntLit " ++ itos ilValue)
    ctx <- gets context
    ye <- liftIO $ C.mkBVConstantFromInteger ctx width ilValue
    return (ye, SBits width)

convAExpr2CExpr mty e@(ASDef (ATBit width) aid) = do
    when traceConv $ traceM("conv def " ++ ppString aid)
    ymap <- gets defCExprMap
...
-}
          ...
          Nothing -> do
            ...
            if (doHardFail)
              then internalError ("convAExpr2CExpr: missing def: " ++
                                  ppReadable aid)
              else addUnknownExpr mty e width

convAExpr2CExpr mty (ASPort (ATBit width) aid) = do
...
convAExpr2CExpr mty (ASParam t@(ATBit _) aid) =
    convAExpr2CExpr mty (ASPort t aid)
...
convAExpr2CExpr mty (ASAny (ATBit width) _) = do
    when traceConv $ traceM("conv any")
    str <- getAnyName
    makeDeclAndVar mty str width

convAExpr2CExpr mty (APrim i (ATBit width) p args) = do
    when traceConv $ traceM("conv prim " ++ ppString p)
    convPrim2CExpr mty p i width args
...
convAExpr2CExpr mty (AMethCall ty@(ATBit width) modId methId args) = do
    smap <- gets stateMap
    let e = AMethCall ty modId (getSingleMethodOutputPort smap modId methId) args
    addUnknownExpr mty e width
convAExpr2CExpr mty (AMethValue ty@(ATBit width) modId methId) = do
    smap <- gets stateMap
    let e = AMethValue ty modId (getSingleMethodOutputPort smap modId methId)
    addUnknownExpr mty e width
convAExpr2CExpr mty (ATupleSel ty@(ATBit width) (AMethCall _ modId methId args) selIdx) = do
    smap <- gets stateMap
    let e = AMethCall ty modId (getMethodOutputPortAt smap modId methId selIdx) args
    addUnknownExpr mty e width
convAExpr2CExpr mty (ATupleSel ty@(ATBit width) (AMethValue _ modId methId) selIdx) = do
    smap <- gets stateMap
    let e = AMethValue ty modId (getMethodOutputPortAt smap modId methId selIdx)
    addUnknownExpr mty e width

convAExpr2CExpr mty e@(AMGate (ATBit 1) _ _) =
...
-- XXX Passing the current context is just a heuristic
convAExpr2CExpr mty e@(ASStr _ (ATString (Just width)) _ ) =
    addUnknownExpr mty e width
convAExpr2CExpr mty e@(ANoInlineFunCall (ATBit width) _ _ _ ) =
    -- XXX This could be an unevaluated function, applied to converted arguments
    addUnknownExpr mty e width
convAExpr2CExpr mty e@(AFunCall (ATBit width) _ _ _ _ ) =
    -- XXX This could be an unevaluated function, applied to converted arguments
    addUnknownExpr mty e width
convAExpr2CExpr mty e@(ATaskValue (ATBit width) _ _ _ _) =
    addUnknownExpr mty e width

-- For unknown abstract types, error out
convAExpr2CExpr _ e =
   internalError ("unexpected expr/type in convAExpr2CExpr: " ++ show e)


convAExpr2CExpr_Force :: Bool -> AExpr -> CM (C.Expr, SType)
convAExpr2CExpr_Force True e = convAExpr2CExpr (Just SBool) e >>= toBool
convAExpr2CExpr_Force False e =
    convAExpr2CExpr (Just (getBitType e)) e >>= toBits

convPrim2CExpr :: Maybe SType ->
                  PrimOp -> AId -> Integer -> [AExpr] -> CM (C.Expr, SType)
convPrim2CExpr mty PrimIf _ _ [c, t, f] = do
    -- force "c" to be Bool
    (yc, _) <- convAExpr2CExpr_Force True c
    res0_t@(y0_t, ty0_t) <- convAExpr2CExpr mty t
    res0_f@(y0_f, ty0_f) <- convAExpr2CExpr mty f
    ...
    ctx <- gets context
    yif <- liftIO $ C.mkIte ctx yc yt yf
    return (yif, ty)

-- XXX what if the type of the case is String etc?
convPrim2CExpr mty PrimCase i w (idx : dflt : ces) =
    ...
    in  convAExpr2CExpr mty (foldr foldFn dflt (makePairs ces))
{-
    -- force "idx" to be Bits
    (yidx, _) <- convAExpr2CExpr_Force False idx
    let (cs, es) = split (makePairs ces)
    -- force the conditions to be Bits
    ycs <- mapM (convAExpr2CExpr_Force False) cs >>= mapM (return . fst)
    -- convert the arms
    res0_es <- mapM (convAExpr2CExpr mty) es
    -- convert the default
    res0_dflt@(y0_dflt, ty0_dflt) <- convAExpr2CExpr mty dflt
                ...
                let mty_arms =
                        ...
                        then mty
                                ...
                                _ -> -- can't be no arms if they differ!
                                     internalError ("convPrim2CExpr: case")
...
convPrim2CExpr mty PrimArrayDynSelect i w args =
    case args of
      [APrim _ _ PrimBuildArray es, idx] ->
          ...
          let foldFn (n, e) res =
              ...
              max_idx = case idx_ty of
                          ATBit sz -> (2^sz) - 1
                          _ -> internalError ("convPrim2CExpr: idx_ty")
              arms = zip [0..max_idx] es
              -- default (even if it's not reachable)
              dflt = ASAny (ATBit w) Nothing
          in  convAExpr2CExpr mty (foldr foldFn dflt arms)
      [ASDef _ i_def, idx] -> do
          dmap <- gets defMap
          case (M.lookup i_def dmap) of
            Just (ADef { adef_expr = e_def }) ->
                convPrim2CExpr mty PrimArrayDynSelect i w [e_def, idx]
            _ -> internalError ("convPrim2CExpr: PrimArrayDynSelect: " ++
                                ppReadable args)
      _ -> internalError ("convPrim2CExpr: PrimArrayDynSelect: " ++
                          ppReadable args)

-- Arguments must be the same type (either Bits or Bool), result is Bool
convPrim2CExpr _ PrimEQ   _ _ args =
  case args of
    [arg1, arg2] -> do
        -- force the arms to be of the same type
        (y_arg1, ty_arg1) <- convAExpr2CExpr Nothing arg1
        (y_arg2, _) <- convAExpr2CExpr (Just ty_arg1) arg2
                           >>= convType (Just ty_arg1)
        ctx <- gets context

        -- STP's mkEq segfaulted on Bool inputs, so use iff (cvc5 handles either)
        let eqFn = if (ty_arg1 == SBool) then C.mkIff else C.mkEq

        yeq <- liftIO $ eqFn ctx y_arg1 y_arg2
        return (yeq, SBool)
    _ -> internalError ("convPrim2CExpr: PrimEQ: wrong number of args: " ++
                        ppReadable args)

-- Bool arguments, Bool result
convPrim2CExpr _ PrimBOr  _ w args = doBinManyPrim_BoolBool C.mkOrMany args
convPrim2CExpr _ PrimBAnd _ w args = doBinManyPrim_BoolBool C.mkAndMany args
convPrim2CExpr _ PrimBNot _ w args = doUnPrim_BoolBool C.mkNot args

-- Bit arguments, Bool result
convPrim2CExpr _ PrimULE  _ w args = doBinPrim_BitsBool C.mkBVLe args
convPrim2CExpr _ PrimULT  _ w args = doBinPrim_BitsBool C.mkBVLt args
convPrim2CExpr _ PrimSLE  _ w args = doBinPrim_BitsBool C.mkBVSle args
convPrim2CExpr _ PrimSLT  _ w args = doBinPrim_BitsBool C.mkBVSlt args

-- Bit arguments, Bit result
convPrim2CExpr _ PrimAnd  _ w args = doBinPrim_BitsBits w C.mkBVAnd args
convPrim2CExpr _ PrimOr   _ w args = doBinPrim_BitsBits w C.mkBVOr args
convPrim2CExpr _ PrimXor  _ w args = doBinPrim_BitsBits w C.mkBVXor args
-- bitwise negation
convPrim2CExpr _ PrimInv  _ w args = doUnPrim_BitsBits w C.mkBVNot args

-- Bit arguments, Bit result
--   cvc5 requires the arguments to be the same size
...
convPrim2CExpr _ PrimAdd  _ w args = doBinPrimSz_BitsBits w C.mkBVAdd args
convPrim2CExpr _ PrimSub  _ w args = doBinPrimSz_BitsBits w C.mkBVSub args
convPrim2CExpr _ PrimMul  _ w args =
    let args' = map (aZeroExtend w) args
    in  doBinPrimSz_BitsBits w C.mkBVMul args'
convPrim2CExpr _ PrimQuot _ w args =
    let arg_size = maximum $ map (getWidth . ae_type) args
        args' = map (aZeroExtend arg_size) args
    in  doBinPrimSz_BitsBits arg_size C.mkBVDiv args' >>= sTruncate w
convPrim2CExpr _ PrimRem  _ w args =
    let arg_size = maximum $ map (getWidth . ae_type) args
        args' = map (aZeroExtend arg_size) args
    in  doBinPrimSz_BitsBits arg_size C.mkBVMod args' >>= sTruncate w
-- arith negation
convPrim2CExpr _ PrimNeg  _ w args = doUnPrim_BitsBits w C.mkBVMinus args

-- Bit argument, static Int argument, Bit result
convPrim2CExpr _ PrimSL _ w [e, n@(ASInt {})] =
    doBinIntPrim_BitsBits w C.mkBVShiftLeft e (extractInt n)
convPrim2CExpr _ PrimSRL i w [e, n@(ASInt {})] =
    doBinIntPrim_BitsBits w C.mkBVShiftRight e (extractInt n)

-- Dynamic Shifting must extend all arguments to same size
convPrim2CExpr mty PrimSL  i w args =
  let arg_size = maximum $ map (getWidth . ae_type) args
      args' = map (aZeroExtend arg_size) args
  in  doBinPrimSz_BitsBits arg_size C.mkBVShiftLeftExpr args' >>= sTruncate w

convPrim2CExpr mty PrimSRL i w args =
  let arg_size = maximum $ map (getWidth . ae_type) args
      args' = map (aZeroExtend arg_size) args
  in  doBinPrimSz_BitsBits arg_size C.mkBVShiftRightExpr args' >>= sTruncate w

-- For signed shift right, the left operator must be sign extended...
convPrim2CExpr mty PrimSRA i w [e1,e2] =
  let arg_size = maximum $ map (getWidth . ae_type) [e1,e2]
      e1' = aSignExtend arg_size e1
      e2' = aZeroExtend arg_size e2
  in  doBinPrimSz_BitsBits arg_size C.mkBVSignedShiftRightExpr [e1',e2'] >>= sTruncate w


-- Bit argument, identical static Int arguments, Bool result
convPrim2CExpr (Just SBool) PrimExtract _ _
               [e, ub@(ASInt {}), lb@(ASInt {})] | (yub == ylb) = do
    (ye, _) <- convAExpr2CExpr_Force False e
    ctx <- gets context
    yext <- liftIO $ C.mkBVBoolExtract ctx ylb ye
    return (yext, SBool)
  where yub = extractInt ub
        ylb = extractInt lb
-- Bit argument, static Int arguments, Bit result
convPrim2CExpr _ PrimExtract _ _ [e, ub@(ASInt {}), lb@(ASInt {})] = do
    ...
    (ye, _) <- convAExpr2CExpr_Force False e
    ctx <- gets context
    yext <- liftIO $ C.mkBVExtract ctx ylb yub ye
    return (yext, ty)
-- Don't handle dynamic extraction
-- XXX We could implement the logic for this?
convPrim2CExpr mty PrimExtract i w args =
    -- XXX This could be an unevaluated function, applied to converted arguments
    addUnknownExpr mty (APrim i (ATBit w) PrimExtract args) w

-- Bit arguments, Bit result
convPrim2CExpr _ PrimConcat i w args@[_, _] =
    doBinPrim_BitsBits w C.mkBVConcat args
convPrim2CExpr _ PrimConcat i width (a1:args) =
    let a1width = case (ae_type a1) of
                   ATBit n -> n
                   t -> internalError ("convPrim2CExpr: PrimConcat width: " ++
                                       ppReadable t)
        a2width = width - a1width
        a2 = APrim i (ATBit a2width) PrimConcat args
    in  doBinPrim_BitsBits width C.mkBVConcat [a1, a2]

-- Sign extend,  sizes take from argument and return type
convPrim2CExpr _ PrimSignExt i w [e] =
    doBinIntPrim_BitsBits w C.mkBVSignExtend e (fromInteger w)

convPrim2CExpr mty p i w args =
...
doBinPrimSz_BitsBits :: Integer
                     -> (C.Context -> Int -> C.Expr -> C.Expr -> IO C.Expr)
                     -> [AExpr]
                     -> CM (C.Expr, SType)
...
doBinIntPrim_BitsBits :: Integer
                      -> (C.Context -> C.Expr -> Int -> IO C.Expr)
                      -> AExpr
                      -> Int
                      -> StateT CState IO (C.Expr, SType)
...
boolArgs :: [AExpr] -> CM [C.Expr]
boolArgs = mapM (\ a -> convAExpr2CExpr_Force True a >>= return . fst)
bitsArgs :: [AExpr] -> CM [C.Expr]
bitsArgs = mapM (\ a -> convAExpr2CExpr_Force False a >>= return . fst)
...
doUnPrim :: (C.Context -> C.Expr -> IO C.Expr) -> [C.Expr] -> CM C.Expr
...
doBinPrim :: (C.Context -> C.Expr -> C.Expr -> IO C.Expr) ->
             [C.Expr] -> CM C.Expr
...
doBinManyPrim :: (C.Context -> [C.Expr] -> IO C.Expr) ->
                 [C.Expr] -> CM C.Expr
...
doBinIntPrim :: (C.Context -> C.Expr -> Int -> IO C.Expr) ->
                C.Expr -> Int -> CM C.Expr
...
getWidth (ATBit w) = w
getWidth t = internalError ("AExpr2CVC5.getWidth: " ++ ppReadable t)
...
aZeroExtend w e =
    ...
    in  case (compare pad_size 0) of
          ...
          LT -> internalError ("AExpr2CVC5.aZeroExtend: " ++ ppReadable (w, e))
...
aSignExtend w e =
    ...
    in  case (compare pad_size 0) of
          ...
          LT -> internalError ("AExpr2CVC5.aSignExtend: " ++ ppReadable (w, e))

sTruncate :: Integer -> (C.Expr, SType) -> CM (C.Expr, SType)
sTruncate w res@(ye, SBits e_sz) | w == e_sz = return res
sTruncate w (ye, SBits e_sz) | e_sz > w =
    do ctx <- gets context
       ytrunc <- liftIO $ C.mkBVExtract ctx 0 (fromInteger (w-1)) ye
       return (ytrunc, SBits w)
sTruncate w (ye, yt) =
    internalError ("AExpr2CVC5.sTruncate: " ++ ppReadable (w, yt))
